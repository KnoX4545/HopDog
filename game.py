# games.py - منطق بازی‌ها (XO - دوز) - نسخه اصلاح شده
...
class GameXO:
    ...
    def __init__(self, host_id: int, host_name: str, bet_amount: int):
        import time
        # ایجاد game_id منحصر به فرد با میلی‌ثانیه
        self.game_id = f"xo_{host_id}_{int(time.time() * 1000)}"
...
class GameManager:
    ...
    def create_game(self, host_id: int, host_name: str, bet_amount: int) -> Tuple[bool, str, Optional[GameXO]]:
        ...
        logger.info(f"✅ بازی ساخته شد - game_id: {game.game_id}, تعداد بازی‌های فعال: {len(self.games)}")
