# bot.py - فایل اصلی بات تلگرام (نسخه نهایی - کاملاً درست)
    ...
    if is_group:
        ...
        if text_lower in ["تغییر اسم", "اسم هاپویی"]:
            if game.data["hop_point"] < 750:
                ...
                return
            msg = await update.message.reply_text("✏️ اسم جدید خود را وارد کن (روی این پیام ریپلای کن):")
        ...
        if context.user_data.get("waiting_for_new_name", False):
            if update.message.reply_to_message and update.message.reply_to_message.message_id == context.user_data.get("name_msg_id"):
                new_name = text
                ...
                await update.message.reply_text("❌ لطفاً روی پیام بات ریپلای کن و اسم جدید رو وارد کن")
            return
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "hapo_rename":
        if game.data["hop_point"] < 750:
            ...
            return
        msg = await query.edit_message_text("✏️ اسم جدید هاپو رو وارد کن (روی این پیام ریپلای کن):")
        context.user_data["waiting_for_hapo_name"] = True
        context.user_data["hapo_name_msg_id"] = msg.message_id
        return
    ...
    if data == "bank_deposit":
        msg = await query.edit_message_text("💰 مبلغ واریزی رو بنویس (روی این پیام ریپلای کن):")
        context.user_data["bank_deposit"] = True
        context.user_data["bank_msg_id"] = msg.message_id
        return
    
    if data == "bank_withdraw":
        msg = await query.edit_message_text("💰 مبلغ برداشت رو بنویس (روی این پیام ریپلای کن):")
...
async def handle_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("waiting_for_hapo_name", False):
        if update.message.reply_to_message and update.message.reply_to_message.message_id == context.user_data.get("hapo_name_msg_id"):
            new_name = text
            if game.data["hop_point"] < 750:
    ...
    if context.user_data.get("bank_deposit", False):
        if update.message.reply_to_message and update.message.reply_to_message.message_id == context.user_data.get("bank_msg_id"):
            try:
            ...
            await update.message.reply_text("❌ لطفاً روی پیام بات ریپلای کن و مبلغ رو وارد کن")
