# bot.py - فایل اصلی بات تلگرام (نسخه نهایی - بدون پوشه)
...
# فایل‌ها در ریشه پروژه ذخیره می‌شن
...
def get_user_display(user_id, username, full_name):
    if username:
        return f"@{username}"
    else:
        return full_name or f"کاربر{user_id}"

# ================================================================
# متن‌های راهنما (آکادمی) - فقط بخش اصلی برای فشردگی
# ================================================================
...
# (بقیه متن‌های آکادمی مثل قبل هستند - برای فشردگی حذف شدند)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    display_name = get_user_display(user_id, username, full_name)
    ...
    if is_group:
        ...
        if text_lower in ["تغییر اسم", "اسم هاپویی"]:
            if game.data["hop_point"] < 750:
                ...
                return
            await update.message.reply_text("✏️ اسم جدید خود را وارد کن (روی این پیام ریپلای کن):")
        ...
        if context.user_data.get("waiting_for_new_name", False):
            # بررسی اینکه ریپلای روی پیام بات هست
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "hapo_rename":
        if game.data["hop_point"] < 750:
            ...
            return
        await query.edit_message_text("✏️ اسم جدید هاپو رو وارد کن (روی این پیام ریپلای کن):")
    ...
    if data == "bank_deposit":
        await query.edit_message_text("💰 مبلغ واریزی رو بنویس (روی این پیام ریپلای کن):")
        context.user_data["bank_deposit"] = True
        return
    
    if data == "bank_withdraw":
        await query.edit_message_text("💰 مبلغ برداشت رو بنویس (روی این پیام ریپلای کن):")
