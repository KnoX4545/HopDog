...
from games import game_manager, GameXO
...
# ================================================================
# توابع کمکی برای نمایش نام کاربر
# ================================================================

def get_user_display_name(user_id, username="", full_name=""):
    """دریافت نام نمایشی کاربر با اولویت: full_name > username > user_id"""
...
def get_user_display_name_from_game(game):
    """دریافت نام نمایشی از روی game.data"""
    user_id = game.user_id
    player_name = game.data.get("player_name", "")
    
    if player_name and not player_name.startswith("کاربر"):
        return player_name
...
# ================================================================
# تابع do_hunt با context
# ================================================================
async def do_hunt(update: Update, context: ContextTypes.DEFAULT_TYPE, game: HopDogGame):
    if game.is_jailed():
        await update.message.reply_text("⛓️ *شما در زندان هستید و نمی‌توانید این کار را انجام دهید.*", parse_mode="Markdown")
        return
...
async def show_fridge_item_detail(update: Update, context: ContextTypes.DEFAULT_TYPE, query, index):
    ...
    if cooking:
        ...
        keyboard.append([
            InlineKeyboardButton(f"🔥 بپوخش ({int(weight * 100)} ثانیه)", callback_data=f"fridge_cook_{index}")
        ])
        keyboard.append([
            InlineKeyboardButton(f"💰 فروش ({format_number(value)} 🪙)", callback_data=f"fridge_sell_{index}")
        ])
    ...
    keyboard.append([InlineKeyboardButton("◀️ برگشت به یخچال", callback_data="fridge_back")])
...
def get_xo_board_keyboard(game, user_id: int):
    ...
    if game.status == "finished":
        ...
        keyboard.append([InlineKeyboardButton("🗑️ لغو میز", callback_data=f"xo_cancel_{game.game_id}")])
    
    return InlineKeyboardMarkup(keyboard)


def get_xo_game_text(game) -> str:
    """دریافت متن وضعیت بازی XO"""
    ...
    if game.status == "finished":
        if game.winner == "draw":
            msg += f"🤝 *نتیجه:* مساوی!\n"
            msg += f"💰 *پوینت‌ها به صاحبانش برگشت*\n"
        else:
            winner_name = game.host_name if game.winner == "host" else game.player_name
            prize = game.bet_amount * 2
            msg += f"🏆 *برنده:* {winner_name}!\n"
            msg += f"💰 *جایزه:* {format_number(prize)} 🪙\n"
    else:
...
async def show_games_menu(update: Update, game_obj):
    ...
    try:
        ...
        keyboard = [
            [InlineKeyboardButton("🧩 بازی XO", callback_data="game_xo_main")]
        ]
        await update.message.reply_text(msg, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
    except Exception as e:
...
async def show_xo_main(update: Update, query=None, game_obj=None):
    ...
    if on_cooldown:
        await query.edit_message_text(f"⏳ *لطفاً {remaining} ثانیه صبر کنید تا دوباره بتوانید بازی کنید.*", parse_mode="Markdown")
        return
    ...
    if str(game_obj.user_id) in GAME_XO_STATE:
        ...
        if state.get("state") == "betting":
            ...
            msg += "❓ شما درحال تعیین مبلغ ورودی بازی XO میباشید.\n\n"
            msg += "🔺 لطفا مبلغ ورودی را در جواب همین پنل وارد کنید\n┘─ مثال :\n500"
            await query.edit_message_text(msg, parse_mode="Markdown")
            return
    
    msg += "💰 *مبلغ ورودی : تعیین نشده ❌*"
    keyboard = [
        [InlineKeyboardButton("💰 تعیین مبلغ ورودی", callback_data="game_xo_set_bet")]
    ]
    await query.edit_message_text(msg, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")


async def handle_xo_set_bet(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """تنظیم مبلغ شرط‌بندی XO"""
    query = update.callback_query
    await query.answer()
...
async def process_xo_bet(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if bet_amount < 50:
        await update.message.reply_text(f"❌ *حداقل مبلغ شرط‌بندی {50} هاپو پوینت است.*", parse_mode="Markdown")
        return
    if bet_amount > 1000000:
        await update.message.reply_text(f"❌ *حداکثر مبلغ شرط‌بندی {1000000} هاپو پوینت است.*", parse_mode="Markdown")
        return
    ...
    keyboard = [
        [InlineKeyboardButton(f"🏗️ ساخت میز بازی ({format_number(bet_amount)} 🪙)", callback_data=f"game_xo_create_{bet_amount}")]
    ]
    await update.message.reply_text(msg, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")


async def handle_xo_create(update: Update, context: ContextTypes.DEFAULT_TYPE, bet_amount: int):
    """ساخت میز بازی XO - ارسال پیام جدید با دکمه پیوستن"""
        ...
        if query.message.chat.type in ["group", "supergroup"]:
            ...
            await context.bot.send_message(
                ...
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🧩 پیوستن به بازی", callback_data=f"game_xo_join_{game_id}")]
                ]),
                parse_mode="Markdown"
            )
    except Exception as e:
...
async def handle_xo_join(update: Update, context: ContextTypes.DEFAULT_TYPE, game_id: str):
    ...
    if hop_point < game.bet_amount:
        await query.edit_message_text(f"❌ *موجودی کافی نیست. شما {format_number(hop_point)} هاپو پوینت داری. نیاز به {format_number(game.bet_amount)} 🪙 داری.*", parse_mode="Markdown")
        return
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if data == "game_xo_set_bet":
            ...
            return
        if data.startswith("game_xo_create_"):
            bet_amount = int(data.replace("game_xo_create_", ""))
            await handle_xo_create(update, context, bet_amount)
            return
        if data.startswith("game_xo_join_"):
            game_id = data.replace("game_xo_join_", "")
            await handle_xo_join(update, context, game_id)
            return
        if data.startswith("xo_move_"):
            parts = data.replace("xo_move_", "").split("_")
            if len(parts) == 3:
                game_id = parts[0]
                row = int(parts[1])
                col = int(parts[2])
            ...
            return
        if data.startswith("xo_close_"):
            game_id = data.replace("xo_close_", "")
        ...
        if data == "hunt_sell":
            ...
            if result["success"]:
                await query.message.reply_text(f"💰 *حیوان فروخته شد!*\n✅ *{format_number(result['value'])} هاپو پوینت دریافت کردی*", parse_mode="Markdown")
            else:
                await query.answer(f"❌ {result['reason']}", show_alert=True)
            return
            ...
            if error_msg == "هاپو سیر است" and animal:
                if game._to_float(game.data.get("hunt_time", 0)) > 0:
                    ...
                    if (now - hunt_time) > HUNT_DECISION_TIMER:
                        ...
                        game.data["hunt_time"] = "0"
                        game.save_data()
                        await query.message.reply_text("🦌 *حیوان فرار کرد! وقتت تموم شد.*", parse_mode="Markdown")
                        return
        ...
        if data == "bank_deposit":
            await query.edit_message_text("💰 *مبلغ واریزی رو بنویس:*\n\n💡 *فقط عدد مبلغ رو تایپ کن و ارسال کن.*", parse_mode="Markdown")
