...
from academy import (
    ...
    ACADEMY_GAMES_MENU, ACADEMY_GAMES_XO,
...
from globals import (
    get_game, get_street_hapo,
    SPAM_TRACKER, MEOW_VOTES, TRANSFER_STATE,
    STREET_HAPO_LAST_SENT, GAME_XO_STATE, GAME_MESSAGES,
    save_game_message, get_game_message, clear_game_message,
    set_xo_state, get_xo_state, clear_xo_state,
    clear_user_game
)
from utils import (
    format_number, parse_amount, get_confirm_keyboard,
    get_cooldown_text, safe_int, safe_str, safe_float,
    format_duration, get_time_remaining
)
from logger_config import (
    log_transaction, log_security, log_game, log_db,
    log_error, log_stats
)
...
async def show_bank_menu(update: Update, game):
    ...
    if level < 4:
        await update.message.reply_text(
            f"🏦 *بانک هاپویی از سطح 4 باز میشود*\n"
            f"⭐ *سطح شما:* {level}",
            parse_mode="Markdown"
        )
        return
...
async def set_user_level(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    log_security(user_id, "تغییر سطح", f"کاربر {user_data['user_id']}: {old_level} → {new_level}")
...
async def add_user_level(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    log_security(user_id, "افزایش سطح", f"کاربر {user_data['user_id']}: +{add_amount} → {new_level}")
...
async def set_user_point(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    log_security(user_id, "تنظیم پوینت", f"کاربر {user_data['user_id']}: {old_point} → {new_point}")
...
async def add_user_point(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    log_security(user_id, "افزایش پوینت", f"کاربر {user_data['user_id']}: +{add_amount} → {new_point}")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if is_group:
            print(f"📩 پردازش گروه: '{text_lower}'")
            ...
            if hapo_name and game.data.get("hapo_owned", False):
                ...
                hapo_name_clean = hapo_name_lower.replace(" ", "")
                text_clean = text_lower.replace(" ", "")
                
                print(f"🔍 hapo_name_lower: '{hapo_name_lower}'")
                print(f"🔍 text_lower: '{text_lower}'")
                print(f"🔍 hapo_name_clean: '{hapo_name_clean}'")
                print(f"🔍 text_clean: '{text_clean}'")
                
                if text_lower == hapo_name_lower:
                    print("✅✅✅ HAPO NAME DETECTED (exact)! ✅✅✅")
                    await show_hapo_menu(update, game)
                    return
                
                if text_clean == hapo_name_clean:
                    print("✅✅✅ HAPO NAME DETECTED (no space)! ✅✅✅")
                    await show_hapo_menu(update, game)
                    return
                
                # اگر اسم هاپو داخل متن باشه
                if hapo_name_lower in text_lower:
                    print("✅✅✅ HAPO NAME DETECTED (inside text)! ✅✅✅")
                    await show_hapo_menu(update, game)
                    return
            ...
            if text_lower in ["هاپ", "hop", "واق", "هوپ", "hap"]:
                await do_hop(update, game)
                return
            
            if text_lower in ["هاپ هاپ", "hop hop", "واق واق", "هاپ هوپ", "hap hap"]:
                await do_hop(update, game)
                return
            ...
            print(f"❌ دستور ناشناخته در گروه: '{text_lower}'")
            return
        ...
        if is_private:
            print(f"📩 پردازش پیوی: '{text_lower}'")
            ...
            # ======== دستورات عمومی در پیوی ========
            ...
            elif text_lower in ["هاپ", "hop", "واق", "هوپ", "hap"]:
                await do_hop(update, game)
            
            elif text_lower in ["هاپ هاپ", "hop hop", "واق واق", "هاپ هوپ", "hap hap"]:
                await do_hop(update, game)
                ...
                if hapo_name and game.data.get("hapo_owned", False):
                    ...
                    if text_lower == hapo_name_lower or text_lower.replace(" ", "") == hapo_name_lower.replace(" ", ""):
                        print("✅✅✅ HAPO NAME DETECTED in private! ✅✅✅")
                        await show_hapo_menu(update, game)
                        return
                
                print(f"❌ دستور ناشناخته در پیوی: '{text_lower}'")
    ...
    try:
        ...
        if data == "hunt_sell":
            ...
            if result["success"]:
                ...
                await query.message.reply_text(
                    f"💰 *حیوان فروخته شد!*\n"
                    f"✅ *{format_number(result['value'])} 🪙 دریافت کردی*\n"
                    f"💰 *هاپو پوینت هات:* {format_number(hop_point)} 🪙",
                    parse_mode="Markdown"
                )
            else:
        ...
        if data == "confirm_hapo_name":
            ...
            await query.edit_message_text(
                ...
                f"💰 *هاپو پوینت هات:* {format_number(hop_point - 750)} 🪙",
                parse_mode="Markdown"
            )
            context.user_data["new_hapo_name"] = None
            await asyncio.sleep(2)
